...
Synchronized API and Product Detail Scraping

Coordinates API scraper (link collection) and
product detail scraper (data processing)
...
from espscraper.api_product_detail_scraper import (
    ApiProductDetailScraper,
    ScrapingConfig,
)
...
def validate_environment():
    ...
    os.environ.setdefault(
        ...
        "https://espweb.asicentral.com/Default.aspx?appCode=WESP&appVersion=4.1.0&page=ProductDetails&referrerPage=ProductResults&referrerModule=PRDRES&refModSufx=Generic&PCUrl=1&productID={product_id}&autoLaunchVS=0&tab=list",
    )
...
def check_link_collection_needed(logger, args):
    ...
    if file_age > (max_age_hours * 3600):
        logger.info(
            f"⏰ Links file is {file_age/3600:.1f} hours old (max: {max_age_hours}h), link collection needed"
        )
        return True

    logger.info(
        f"✅ Links file is fresh ({file_age/3600:.1f}h old),\
        skipping link collection"
    )
    return False
...
def collect_links_phase(logger, session_manager, args):
    ...
    try:
        ...
        logger.info(
            f"📊 Starting link collection with batch_size={args.batch_size},\
...
def process_products_phase(logger, session_manager, args):
    ...
    try:
        ...
        logger.info(
            f"📊 Starting product processing with batch_size={detail_config.batch_size}, max_retries={detail_config.max_retries}"
        )

        detail_scraper.scrape_all_products(mode=args.mode, limit=args.product_limit)
...
def run_synchronized_scraping(args):
    ...
    try:
        ...
        if check_link_collection_needed(logger, args):
            ...
            if not link_success:
                if args.fail_on_link_collection:
                    logger.error(
                        "❌ Link collection phase failed and --fail-on-link-collection is set"
                    )
                    return False
                else:
                    logger.warning(
                        "⚠️ Link collection failed, but continuing with existing links"
                    )
        ...
        logger.info(
            f"📊 Configuration: batch_size={args.batch_size}, max_retries={args.max_retries}"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="ESP Scraper - Synchronized Production Version"
    )
    ...
    parser.add_argument(
        "--link-limit", type=int, help="Limit number of links to collect"
    )
    ...
    parser.add_argument(
        "--product-limit", type=int, help="Limit number of products to process"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="Batch size for both scrapers (default: 20)",
    )
    parser.add_argument(
        "--max-retries",
        type=int,
        default=3,
        help="Maximum retries for both scrapers (default: 3)",
    )
    parser.add_argument(
        "--force-relogin", action="store_true", help="Force fresh login"
    )
    ...
    if args.dry_run:
        ...
        print(
            f"📊 Link collection: {'forced' if args.force_link_collection else 'conditional'}"
        )
