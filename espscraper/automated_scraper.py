...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("automated_scraper.log"), logging.StreamHandler()],
)
...
class AutomatedScraper:
    ...
    def start(self):
        ...
        schedule.every(self.config.scrape_interval_hours).hours.do(
        ...
        logging.info(
            f"✅ Automated scraper started. Scraping every {self.config.scrape_interval_hours} hours at {self.config.scrape_time}"
        )
    ...
    def _get_latest_product_file(self) -> Optional[str]:
        ...
        files = [
            ...
            if f.startswith("api_product_details_") and f.endswith(".jsonl")
        ]
        if not files:
            return None
    ...
    def run_scraping_job(self):
        ...
        try:
            ...
            if len(product_links) > self.config.max_products_per_run:
                ...
                logging.info(
                    f"📦 Limited to {self.config.max_products_per_run} products for this run"
                )

            # Create output file with timestamp
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            output_file = os.path.join(
                self.config.data_directory, f"api_product_details_{timestamp}.jsonl"
            )
            ...
            latest_file = os.path.join(
                self.config.data_directory, "api_product_details_latest.jsonl"
            )
            ...
            logging.info(
                f"✅ Scraping job completed: {products_scraped} products in {duration:.1f}s"
            )
    ...
    def _scrape_products(self, product_links: List[Dict], output_file: str) -> int:
        ...
        with open(output_file, "w", encoding="utf-8") as f_out:
            for i, link_info in enumerate(product_links):
                ...
                if i % 100 == 0:
                    logging.info(
                        f"📦 Progress: {i}/{len(product_links)} products processed"
                    )
    ...
    def _heartbeat_monitor(self):
        ...
        heartbeat_file = os.path.join(
            self.config.data_directory, "scraper_heartbeat.txt"
        )
        ...
        while self.running:
            try:
                heartbeat_data = {
                    ...
                    "total_products_scraped": self.scraping_stats[
    ...
    def get_scraping_stats(self) -> Dict:
        ...
        return {
            ...
            "last_scrape_time": (
                self.last_scrape_time.isoformat() if self.last_scrape_time else None
            ),
...
def main():
    ...
    parser.add_argument(
        "--scrape-now", action="store_true", help="Run a scraping job immediately"
    )
    parser.add_argument(
        "--interval-hours", type=int, default=6, help="Scraping interval in hours"
    )
    parser.add_argument(
        "--max-products", type=int, default=1000, help="Maximum products per run"
    )
    parser.add_argument(
        "--batch-size", type=int, default=15, help="Batch size for processing"
    )
