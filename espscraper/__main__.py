...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="ESP Product Detail Scraper (Production Entry Point)"
    )
    parser.add_argument(
        "--collect-links",
        action="store_true",
        help="Run link scraper before detail scraper (uses --limit for number of links)",
    )
    parser.add_argument(
        "--limit",
        type=int,
        default=None,
        help="Limit the number of products to scrape (and links to collect if --collect-links is set)",
    )
    ...
    parser.add_argument(
        "--clear-session",
        action="store_true",
        help="Clear session/cache before running",
    )
    parser.add_argument(
        "--new-only",
        action="store_true",
        help="Only collect links for products not already scraped (based on the detail output file)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--debug-mode",
        action="store_true",
        help="Enable real-time connection monitoring and detailed logging",
    )
    ...
    try:
        if args.collect_links:
            ...
            if status and status.get("all_links_collected"):
                logging.info(
                    "All links already collected. Proceeding to detail scraping."
                )
            elif status:
                logging.info(
                    f"New links collected: {status.get('new_links_collected', 0)}"
                )
        ...
        session_manager = SessionManager(
            cookie_file="tmp/session_cookies.json", state_file="tmp/session_state.json"
        )
