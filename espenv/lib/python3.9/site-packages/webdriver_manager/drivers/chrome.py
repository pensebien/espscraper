...
class ChromeDriver(Driver):
    ...
    def get_driver_download_url(self, os_type):
        ...
        if version.parse(driver_version_to_download) >= version.parse("115"):
            ...
            modern_version_url = self.get_url_for_version_and_platform(
                driver_version_to_download, os_type
            )
            log(f"Modern chrome version {modern_version_url}")
            return modern_version_url

        return (
            f"{self._url}/{driver_version_to_download}/{self.get_name()}_{os_type}.zip"
        )
    ...
    def get_latest_release_version(self):
        ...
        if determined_browser_version is not None and version.parse(
            determined_browser_version
        ) >= version.parse("115"):
            url = "https://googlechromelabs.github.io/chrome-for-testing/latest-patch-versions-per-build.json"
            response = self._http_client.get(url)
            response_dict = json.loads(response.text)
            determined_browser_version = (
                response_dict.get("builds")
                .get(determined_browser_version)
                .get("version")
            )
            ...
            latest_release_url = (
                f"{self._latest_release_url}_{determined_browser_version}"
            )
        else:
