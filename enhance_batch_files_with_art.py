#!/usr/bin/env python3
"""
Unified Batch File Processor with Art Template and Image Integration
...
3. Fetching product images from HitPromo product pages
4. Creating FPD configurations based on product data imprint information
"""
...
class ArtTemplateProcessor:
    """Processes art templates and fetches product images - downloads only, no OCR processing"""
    ...
    def get_hitpromo_images(
        self, product_number: str, product_data: Dict = None
    ) -> Dict:
        """Fetch images from HitPromo product page for all color variants"""
        try:
            url = f"{self.base_url}/product/show/{product_number}"
            logging.info(f"🔍 Fetching HitPromo images: {url}")
            ...
            # Fetch images for all color variants
            color_images = {}
            if product_data:
                ...
                for color in colors:
                    ...
                    if vendor_code or sku:
                        # Use vendor code or SKU to generate color-specific URLs
                        color_identifier = vendor_code if vendor_code else sku

                        # Generate possible image URLs for this color
                        color_urls = self._generate_color_image_urls(
                            product_number, color_identifier, color_name
                        )

                        # Test which URLs are valid
                        valid_urls = []
                        for img_url in color_urls:
                            if self._test_image_url(img_url):
                        ...
                        if valid_urls:
                            ...
                            logging.info(
                                f"🎨 Found {len(valid_urls)} images for color {color_name}"
                            )
            ...
            logging.error(
                f"❌ Error fetching HitPromo images for product {product_number}: {e}"
            )
            return {
                "main_image": None,
                "gallery_images": [],
    ...
    def _generate_color_image_urls(
        self, product_number: str, color_identifier: str, color_name: str
    ) -> List[str]:
        """Generate possible image URLs for a specific color"""
        urls = []
        ...
        # Generate URLs with different patterns
        for image_type in image_types:
            # Pattern 1: product_number_color_identifier_image_type.jpg
            url1 = f"{self.base_url}/imageManager/show/{product_number}_{color_identifier}_{image_type}.jpg"
            urls.append(url1)

            # Pattern 2: product_number_color_identifier_image_type.png
            url2 = f"{self.base_url}/imageManager/show/{product_number}_{color_identifier}_{image_type}.png"
            urls.append(url2)

            # Pattern 3: color_identifier_image_type.jpg
            url3 = (
                f"{self.base_url}/imageManager/show/{color_identifier}_{image_type}.jpg"
            )
            urls.append(url3)

            # Pattern 4: color_identifier_image_type.png
            url4 = (
                f"{self.base_url}/imageManager/show/{color_identifier}_{image_type}.png"
            )
            urls.append(url4)

            # Pattern 5: product_number_color_name_image_type.jpg
            url5 = f"{self.base_url}/imageManager/show/{product_number}_{color_name}_{image_type}.jpg"
            urls.append(url5)

            # Pattern 6: product_number_color_name_image_type.png
            url6 = f"{self.base_url}/imageManager/show/{product_number}_{color_name}_{image_type}.png"
            urls.append(url6)
    ...
    def extract_product_number(self, product: Dict) -> Optional[str]:
        """Extract product number from product data - comprehensive extraction"""
        try:
            ...
            for field in possible_fields:
                ...
                if value:
                    ...
                    if isinstance(value, list) and len(value) > 0:
                        ...
                        # If it's a dict, try to get SKU
                        value = value.get("SKU", "")

                    if value:
                        # Clean the product number
                        ...
                        if len(value) > 20:
                            ...
                            if cleaned and len(cleaned) > 4:
                                ...
                                if base_match:
                                    ...
                                    # If no letters after, just return the cleaned number
                                    return cleaned
...
class BatchFileEnhancer:
    ...
    def _enhance_product(self, product: Dict) -> Dict:
        ...
        try:
            ...
            product_number = self.art_processor.extract_product_number(product)
            ...
            # Fetch HitPromo images for all color variants
            hitpromo_images = self.art_processor.get_hitpromo_images(
            ...
            if hitpromo_images.get("color_images"):
                product["color_images"] = hitpromo_images["color_images"]

                # Update colors in attributes with their images
                ...
                # Also update optimized structure colors
                if "optimized" in product:
                    optimized_colors = (
                        product["optimized"].get("attributes", {}).get("colors", [])
                    )
        ...
        try:
            ...
            for i, batch_file in enumerate(batch_files, 1):
                ...
                if success:
                    ...
                    try:
                        # Use enhanced directory in current working directory
                        enhanced_dir = os.path.join(os.getcwd(), "enhanced")
                            ...
                            with open(enhanced_file, "r") as f:
                                for line in f:
                                    if line.strip():
                                        ...
                                        if product.get(
                                            "image_url"
                                        ) and "hitpromo.net" in product.get(
                                            "image_url", ""
                                        ):
                                            processing_stats[
                                                "hitpromo_images_fetched"
                                            ] += 1
                                        if product.get("color_images"):
                                            for color_name, color_data in product[
                                                "color_images"
                                            ].items():
                                                processing_stats[
                                                    "hitpromo_images_fetched"
                                                ] += len(color_data.get("images", []))
                    except Exception as e:
                        logging.warning(
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Enable HitPromo image fetching (default: True)",
    )
    parser.add_argument(
        "--disable-images",
        action="store_true",
        help="Disable HitPromo image fetching",
    )
    ...
    if result["success"]:
        ...
        if result.get("processing_stats"):
            ...
            print(
                f"   🖼️  HitPromo images fetched: {stats.get('hitpromo_images_fetched', 0)}"
            )
